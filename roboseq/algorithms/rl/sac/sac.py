...
from roboseq.algorithms.rl.sac import ReplayBuffer

from roboseq.algorithms.rl.sac import MLPActorCritic
...
class SAC:
    ...
    def __init__(
        ...
        ac_kwargs = dict(
            hidden_sizes=[learn_cfg["hidden_nodes"]] * learn_cfg["hidden_layer"]
        )
        self.actor_critic = MLPActorCritic(
            vec_env.observation_space, vec_env.action_space, **ac_kwargs
        ).to(self.device)
        ...
        self.pi_optimizer = Adam(
            self.actor_critic.pi.parameters(), lr=self.learning_rate
        )
            ...
            for it in range(self.current_learning_iteration, num_learning_iterations):
                ...
                for _ in range(self.num_transitions_per_env):
                    ...
                    self.storage.add_transitions(
                        current_obs, current_states, actions, rews, next_obs, dones
                    )
                    current_obs.copy_(next_obs)
    ...
    def compute_loss_q(self, data):
        ...
        with torch.no_grad():
            ...
            backup = r + self.gamma * (1 - d) * (
                q_pi_targ - self.entropy_coef * logp_a2
            )
