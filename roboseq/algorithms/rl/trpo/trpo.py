...
from roboseq.algorithms.rl.trpo import RolloutStorage
from roboseq.algorithms.rl.trpo import ActorCritic
...
class TRPO:
        ...
        for epoch in range(self.num_learning_epochs):
            ...
            for indices in batch:
                ...
                g = torch.autograd.grad(
                    a_loss, self.actor_critic.actor.parameters(), retain_graph=True
                )
    ...
    def kl_hessian_times_vector(self, v, kl):
        ...
        grad_kl = torch.autograd.grad(
            kl, self.actor_critic.actor.parameters(), create_graph=True
        )
        flat_grad_kl = torch.cat([grad.view(-1) for grad in grad_kl])

        grad_kl_v = (flat_grad_kl * v).sum()
        grad_grad_kl_v = torch.autograd.grad(
            grad_kl_v, self.actor_critic.actor.parameters(), retain_graph=True
        )
