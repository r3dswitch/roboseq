...
from roboseq.algorithms.metarl.maml import RolloutStorage
from roboseq.algorithms.metarl.maml.module import ActorCritic
import TorchOpt
...
class Trainer:
    def __init__(
        ...
        self.meta_storage = RolloutStorage(
            self.env.num_envs,
            self.inner_algo.num_transitions_per_env,
            self.inner_algo.observation_space.shape,
            self.inner_algo.state_space.shape,
            self.inner_algo.action_space.shape,
            self.inner_algo.device,
            self.sampler,
        )
        self.inner_algo = inner_algo
        ...
        self.inner_optim = TorchOpt.MetaSGD(
            self.meta_actor_critic, lr=self.inner_algo.learning_rate
        )
        # self.inner_optim = optim.Adam(self.meta_actor_critic, lr=self.inner_algo.learning_rate)
        self.meta_optimizer = optim.Adam(
            self.meta_actor_critic.parameters(), lr=learning_rate
        )

    def train(self, train_epoch):
        for _ in range(train_epoch):
            support_storage_list = self.inner_algo.sample_support_trajectory(
                self.support_set_size
            )
            query_storage_list = self.inner_algo.sample_query_trajectory(
                self.query_set_size, self.meta_storage
            )
            ...
            with torch.no_grad():
                for i in range(self.task_num):
                    for p_meta, p_inner in zip(
                        self.meta_actor_critic.parameters(),
                        self.inner_algo.actor_critic[i].parameters(),
                    ):
                        polyak = 0
    ...
    def meta_update(self, support_storage_list, query_storage_list):
        ...
        for query_storage in query_storage_list:
            ...
            self.net_state = TorchOpt.extract_state_dict(
                self.meta_actor_critic, enable_visual=True, visual_prefix="step_0."
            )  # extract state
            ...
            for i in range(self.task_num - 1):
                ...
                obs_batch = query_storage.observations[
                    :, self.num_env_each_task * i : self.num_env_each_task * (i + 1), :
                ].reshape(-1, *query_storage.observations.size()[2:])
                if self.asymmetric:
                    states_batch = query_storage.states[
                        :,
                        self.num_env_each_task * i : self.num_env_each_task * (i + 1),
                        :,
                    ].reshape(-1, *query_storage.states.size()[2:])
                else:
                    states_batch = None
                actions_batch = query_storage.actions[
                    :, self.num_env_each_task * i : self.num_env_each_task * (i + 1), :
                ].reshape(-1, query_storage.actions.size(-1))
                target_values_batch = query_storage.values[
                    :, self.num_env_each_task * i : self.num_env_each_task * (i + 1), :
                ].reshape(-1, 1)
                returns_batch = query_storage.returns[
                    :, self.num_env_each_task * i : self.num_env_each_task * (i + 1), :
                ].reshape(-1, 1)
                old_actions_log_prob_batch = query_storage.actions_log_prob[
                    :, self.num_env_each_task * i : self.num_env_each_task * (i + 1), :
                ].reshape(-1, 1)
                advantages_batch = query_storage.advantages[
                    :, self.num_env_each_task * i : self.num_env_each_task * (i + 1), :
                ].reshape(-1, 1)
                old_mu_batch = query_storage.mu[
                    :, self.num_env_each_task * i : self.num_env_each_task * (i + 1), :
                ].reshape(-1, query_storage.actions.size(-1))
                old_sigma_batch = query_storage.sigma[
                    :, self.num_env_each_task * i : self.num_env_each_task * (i + 1), :
                ].reshape(-1, query_storage.actions.size(-1))
                ...
                (
                    ...
                    mu_batch,
                    sigma_batch,
                ) = self.meta_actor_critic.evaluate(obs_batch, None, actions_batch)
            ...
            nn.utils.clip_grad_norm_(
                self.meta_actor_critic.parameters(), self.max_grad_norm
            )
    ...
    def inner_update(self, support_storage, i):
        for epoch in range(self.inner_algo.num_learning_epochs):
            obs_batch = support_storage.observations[
                :, self.num_env_each_task * i : self.num_env_each_task * (i + 1), :
            ].reshape(-1, *support_storage.observations.size()[2:])
            if self.asymmetric:
                states_batch = support_storage.states[
                    :, self.num_env_each_task * i : self.num_env_each_task * (i + 1), :
                ].reshape(-1, *support_storage.states.size()[2:])
            else:
                states_batch = None
            actions_batch = support_storage.actions[
                :, self.num_env_each_task * i : self.num_env_each_task * (i + 1), :
            ].reshape(-1, support_storage.actions.size(-1))
            target_values_batch = support_storage.values[
                :, self.num_env_each_task * i : self.num_env_each_task * (i + 1), :
            ].reshape(-1, 1)
            returns_batch = support_storage.returns[
                :, self.num_env_each_task * i : self.num_env_each_task * (i + 1), :
            ].reshape(-1, 1)
            old_actions_log_prob_batch = support_storage.actions_log_prob[
                :, self.num_env_each_task * i : self.num_env_each_task * (i + 1), :
            ].reshape(-1, 1)
            advantages_batch = support_storage.advantages[
                :, self.num_env_each_task * i : self.num_env_each_task * (i + 1), :
            ].reshape(-1, 1)
            old_mu_batch = support_storage.mu[
                :, self.num_env_each_task * i : self.num_env_each_task * (i + 1), :
            ].reshape(-1, support_storage.actions.size(-1))
            old_sigma_batch = support_storage.sigma[
                :, self.num_env_each_task * i : self.num_env_each_task * (i + 1), :
            ].reshape(-1, support_storage.actions.size(-1))
