...
class MAMLPPO:
    def __init__(
        ...
        for i in range(self.task_num):
            ...
            self.optimizers.append(
                optim.SGD(pseudo_actor_critic[i].parameters(), lr=learning_rate)
            )
        ...
        self.cur_reward_sum = torch.zeros(
            self.num_env_each_task * (self.task_num - 1),
            dtype=torch.float,
            device=self.device,
        )
        self.cur_test_reward_sum = torch.zeros(
            self.num_env_each_task * 1, dtype=torch.float, device=self.device
        )
    ...
    def sample_support_trajectory(self, support_set_size, log_interval=1):
        ...
        for it in range(support_set_size):
            ...
            for _ in range(self.num_transitions_per_env):
                ...
                for i in range(self.task_num):
                    if i == 0:
                        (
                            ...
                            mu,
                            sigma,
                        ) = self.actor_critic[i].act(
                            current_obs[
                                self.num_env_each_task
                                * i : self.num_env_each_task
                                * (i + 1)
                            ],
                            current_states[
                                self.num_env_each_task
                        ...
                        actions_log_prob = torch.cat(
                            (actions_log_prob, tem_actions_log_prob), dim=0
                        )
                ...
                if self.print_log:
                    self.cur_reward_sum[:] += rews[
                        : self.num_env_each_task * (self.task_num - 1)
                    ]
                    self.cur_test_reward_sum += rews[
                        self.num_env_each_task * (self.task_num - 1) :
                    ]
                    self.cur_episode_length[:] += 1

                    new_ids = (
                        dones[: self.num_env_each_task * (self.task_num - 1)] > 0
                    ...
                    for i in range(len(self.vec_env.task_envs) - 1):
                        task_new_ids = (
                            dones[
                                i
                                * self.num_env_each_task : (i + 1)
                                * self.num_env_each_task
                            ]
                            > 0
                        ).nonzero(as_tuple=False)
                        task_reward = self.cur_reward_sum[
                            i
                            * self.num_env_each_task : (i + 1)
                            * self.num_env_each_task
                        ]
                        self.task_reward_sum[i].extend(
                            task_reward[task_new_ids][:, 0].cpu().numpy().tolist()
                        )

                    self.reward_sum.extend(
                        self.cur_reward_sum[new_ids][:, 0].cpu().numpy().tolist()
                    )
                    self.test_reward_sum.extend(
                        self.cur_test_reward_sum[test_new_ids][:, 0]
                        .cpu()
                        .numpy()
                        .tolist()
                    )
                    self.episode_length.extend(
            ...
            for i in range(self.task_num):
                if i == 0:
                    _, _, last_values, _, _ = self.actor_critic[i].act(
                        current_obs[
                            self.num_env_each_task
                            * i : self.num_env_each_task
                            * (i + 1)
                    ...
                    _, _, tem_last_values, _, _ = self.actor_critic[i].act(
                        current_obs[
                            self.num_env_each_task
                            * i : self.num_env_each_task
                            * (i + 1)
    ...
    def log(self, locs, width=80, pad=35, query=False):
            ...
            for i, task_name in enumerate(self.vec_env.task_envs):
                ...
                if len(self.task_rewbuffer[i]) > 0:
                    ...
                    self.writer.add_scalar(
                        "Episode/" + task_name, value, self.train_epoch
                    )
                    ep_string += (
                        f"""{f'Mean Reward {task_name}:':>{pad}} {value:.4f}\n"""
                    )
        ...
        self.writer.add_scalar(
            "Loss/value_function", locs["mean_value_loss"], self.train_epoch
        )
        self.writer.add_scalar(
            "Loss/surrogate", locs["mean_surrogate_loss"], self.train_epoch
        )
        self.writer.add_scalar(
            "Policy/mean_noise_std", mean_std.item(), self.train_epoch
        )
        if len(self.rewbuffer) > 0 and len(self.test_rewbuffer) > 0:
            self.writer.add_scalar(
                "Train/mean_reward", statistics.mean(self.rewbuffer), self.train_epoch
            )
            ...
            self.writer.add_scalar(
                "Train/mean_reward/time", statistics.mean(self.rewbuffer), self.tot_time
            )
            self.writer.add_scalar(
            ...
            self.writer.add_scalar(
                "Train/mean_test_reward",
                statistics.mean(self.test_rewbuffer),
                self.train_epoch,
            )

        self.writer.add_scalar(
            "Train2/mean_reward/step", locs["mean_reward"], self.train_epoch
        )
        self.writer.add_scalar(
    ...
    def update(self):
        ...
        for epoch in range(self.num_learning_epochs):
            ...
            for i in range(self.task_num):
                obs_batch = self.storage.observations[
                    :, self.num_env_each_task * i : self.num_env_each_task * (i + 1), :
                ].reshape(-1, *self.storage.observations.size()[2:])
                if self.asymmetric:
                    states_batch = self.storage.states[
                        :,
                        self.num_env_each_task * i : self.num_env_each_task * (i + 1),
                        :,
                    ].reshape(-1, *self.storage.states.size()[2:])
                else:
                    states_batch = None
                actions_batch = self.storage.actions[
                    :, self.num_env_each_task * i : self.num_env_each_task * (i + 1), :
                ...
                old_actions_log_prob_batch = self.storage.actions_log_prob[
                    :, self.num_env_each_task * i : self.num_env_each_task * (i + 1), :
                ].reshape(-1, 1)
                ...
                (
                    ...
                    mu_batch,
                    sigma_batch,
                ) = self.actor_critic[i].evaluate(obs_batch, None, actions_batch)
                ...
                nn.utils.clip_grad_norm_(
                    self.actor_critic[i].parameters(), self.max_grad_norm
                )
