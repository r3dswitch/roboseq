...
from roboseq.utils.util import get_gard_norm, huber_loss, mse_loss
from roboseq.algorithms.marl.utils.popart import PopArt
from roboseq.algorithms.utils.util import check
from roboseq.algorithms.marl.actor_critic import Actor, Critic
...
class HATRPO:
    ...
    def trpo_update(self, sample, update_actor=True):
        ...
        for i in range(self.ls_step):
            ...
            ratio = torch.exp(
                (action_log_probs - old_action_log_probs_batch).sum(
                    dim=-1, keepdim=True
                )
            )
            if self._use_policy_active_masks:
