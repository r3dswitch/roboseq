...
from roboseq.algorithms.marl.maddpg import MADDPG as TrainAlgo
from roboseq.algorithms.marl.maddpg import MADDPG_policy as Policy
from roboseq.algorithms.marl.maddpg import ReplayBuffer
...
class Runner:
    def __init__(self, vec_env, config, model_dir=""):
        ...
        self.trainer = TrainAlgo(
            config, self.policy, self.num_agents, device=self.device
        )
        for agent_id in range(self.num_agents):
            # buffer
            share_observation_space = self.envs.share_observation_space[agent_id]
    ...
    def run(self):
        ...
        train_episode_rewards = torch.tensor(
            train_episode_rewards, dtype=torch.float, device=self.device
        )
