...
class ReplayBuffer:
    def __init__(
        ...
        self.share_obs = torch.zeros(
            num_transitions_per_env, num_envs, *share_obs_shape, device=self.device
        )
        ...
        self.next_share_obs = torch.zeros(
            num_transitions_per_env, num_envs, *share_obs_shape, device=self.device
        )
        ...
        self.joint_actions = torch.zeros(
            num_transitions_per_env, num_envs, joint_act_dim, device=self.device
        )
        ...
        for _ in range(num_mini_batches):
            if self.fullfill:
                subset = random.sample(
                    range(self.num_transitions_per_env), mini_batch_size
                )
            else:
