...
from roboseq.algorithms.mtrl.mtppo import RolloutStorage
            ...
            for it in range(self.current_learning_iteration, num_learning_iterations):
                ...
                for _ in range(self.num_transitions_per_env):
                    if self.apply_reset:
                        ...
                        current_states = self.vec_env.get_state()
                    # Compute the action
                    actions = 1 - 2 * torch.rand(
                        [self.vec_env.num_envs, self.vec_env.num_actions],
                        dtype=torch.float32,
                        device=self.device,
                    )
                    # Step the vec_environment
                    ...
                    if self.print_log:
                        ...
                        cur_success_sum = torch.where(
                            cur_success_sum > 0,
                            torch.ones_like(cur_success_sum),
                            torch.zeros_like(cur_success_sum),
                        )
                        ...
                        for i, task_name in enumerate(self.vec_env.task_envs):
                            task_new_ids = (
                                dones[
                                    i
                                    * self.vec_env.num_each_envs : (i + 1)
                                    * self.vec_env.num_each_envs
                                ]
                                > 0
                            ).nonzero(as_tuple=False)
                            task_reward = cur_reward_sum[
                                i
                                * self.vec_env.num_each_envs : (i + 1)
                                * self.vec_env.num_each_envs
                            ]
                            task_reward_sum[i].extend(
