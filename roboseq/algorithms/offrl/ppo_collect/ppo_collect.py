...
from roboseq.algorithms.offrl.ppo_collect import RolloutStorage
    ...
    def run(self, num_learning_iterations, log_interval=1):
        ...
        if self.is_testing:
            ...
            while nums < self.data_size:
                ...
                actions_off.append(
                    actions.clone().detach().cpu().numpy().clip(-max_action, max_action)
                )
                rewards_off.append(rews.clone().detach().cpu().numpy().reshape(-1, 1))
                dones_off.append(
                    dones.clone().detach().cpu().numpy().reshape(-1, 1) * 1.0
                )
            ...
            np.save(
                self.data_save + "/next_states.npy", next_states_off[0 : self.data_size]
            )
                ...
                for _ in range(self.num_transitions_per_env):
                    ...
                    if it <= int(num_learning_iterations / 2):
                        index = np.random.choice(
                            self.vec_env.num_envs,
                            int(self.vec_env.num_envs / 10),
                            replace=True,
                        )
                        states_off.append(
                            current_obs[index].clone().detach().cpu().numpy()
                        )
                        next_states_off.append(
                            next_obs[index].clone().detach().cpu().numpy()
                        )
                        actions_off.append(
                            actions[index]
                            .clone()
                        ...
                        rewards_off.append(
                            rews[index].clone().detach().cpu().numpy().reshape(-1, 1)
                        )
                        dones_off.append(
                            dones[index].clone().detach().cpu().numpy().reshape(-1, 1)
                            * 1.0
                        )
