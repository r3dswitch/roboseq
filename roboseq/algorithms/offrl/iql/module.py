...
class TDNetwork(nn.Module):
    def __init__(self, state_dim, action_dim, net_type):
        ...
        layer_list = [
            nn.Linear(input_size, self.layer_sizes[0]),
...
class Policy(nn.Module):
    def __init__(self, state_dim, action_dim, max_action):
        ...
        layer_list = [
            nn.Linear(self.state_dim, self.layer_sizes[0]),
            self.nonlinearity_class(),
        ]
        for i in range(len(self.layer_sizes) - 1):
            layer_list.append(nn.Linear(self.layer_sizes[i], self.layer_sizes[i + 1]))
    ...
    def forward_dist(self, states):
        ...
        return torch.distributions.transformed_distribution.TransformedDistribution(
            normal_dist, [torch.distributions.transforms.TanhTransform(cache_size=1)]
        )
...
class IQL_Model:
    def __init__(
        ...
        self.policy_net = Policy(self.state_dim, self.action_dim, self.max_action).to(
            self.device
        )
    ...
    def expectile_loss(self, value, expectile_prediction):
        ...
        return torch.mean(
            torch.abs(
                self.expectile * torch.square(u) + (nn.functional.relu(-1 * u) * u)
            )
        )
    ...
    def target_update(self):
        for target_param, param in zip(
            self.target_q_net_1.parameters(), self.q_net_1.parameters()
        ):
            target_param.data.copy_(
                (1.0 - self.tau) * target_param.data + self.tau * param.data
            )
        for target_param, param in zip(
            self.target_q_net_2.parameters(), self.q_net_2.parameters()
        ):
            target_param.data.copy_(
                (1.0 - self.tau) * target_param.data + self.tau * param.data
            )
    ...
    def L_pi(self, states, actions):
        ...
        Q_values = torch.minimum(Q_1, Q_2)
        V_values = self.v_net.forward(states)
        exp_advantages = torch.clip(
            torch.exp(self.beta * (Q_values - V_values)), max=100
        )
        action_log_probs = self.policy_net.forward_dist(states).log_prob(
            torch.clamp(actions, min=-0.99, max=0.99)
        )
    ...
    def select_action(self, state):
        with torch.no_grad():
            action = self.max_action * torch.tanh(
                self.policy_net.means(self.policy_net.features(state))
            )
        return action
